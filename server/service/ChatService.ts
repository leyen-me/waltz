...
import ChatNoStremError from "../error/app/chat/ChatNoStremError";
...
export default class ChatService extends BaseService<Chat> {
  ...
  async createChat(
    chatData: CreationAttributes<Chat>
  ): Promise<BaseCreateResponse> {
    const createdChatId = await defineTransactionWrapper(
      async (transaction) => {
  ...
  async updateChat(
    chatId: number,
    chatData: Partial<CreationAttributes<Chat>>
  ): Promise<void> {
  ...
  async deleteChats(chatIds: number[], userId: number): Promise<void> {
    await defineTransactionWrapper(async (transaction) => {
      await this.delete({
        where: { id: chatIds, userId: userId },
        transaction,
      });
    });
  }
  ...
  async getStream(chatId: number, prompt?: string): Promise<ReadableStream> {
    ...
    const stream = await useFetchStream({
      url: getValue(siteConfigs, CONFIG_KEY.CHATGPT.URL),
      authorization: `Bearer ${getValue(siteConfigs, CONFIG_KEY.CHATGPT.KEY)}`,
      body: {
        model: getValue(siteConfigs, CONFIG_KEY.CHATGPT.MODEL),
        messages,
      },
      stream: true,
      success,
