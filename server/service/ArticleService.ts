...
export default class ArticleService extends BaseService<Article> {
    ...
    async selectPage(query: ArticleQuery): Promise<BasePageResponse<Article>> {
        ...
        if (title) {
            whereConditions.push(sequelize.literal('MATCH(title, content) AGAINST(:title IN BOOLEAN MODE)'));
        }

        if (tagId) {
            whereConditions.push(sequelize.literal(`EXISTS (SELECT 1 FROM t_article_tag at WHERE at.article_id = Article.id AND at.tag_id = :tagId)`));
        }
        ...
        const options: FindAndCountOptions = {
            ...
            attributes: [
                ...
                [sequelize.literal('(SELECT GROUP_CONCAT(t1.title) FROM t_tag t1 LEFT JOIN t_article_tag t2 ON t1.id = t2.tag_id WHERE t2.article_id = Article.id)'), 'tagList'],
                'authorId',
                'categoryId',
                'publishedAt',
                'viewsCount',
    ...
    async getArticleById(articleId: number, isCount: boolean): Promise<Article | null> {
        const query = `
            SELECT a.*, 
